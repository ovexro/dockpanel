...
> **Version**: v2.229.0 | **Total**: 60+ major features, ~285 capabilities
>
...
| **Auto-Optimization** | PHP-FPM worker analysis, nginx workers vs CPUs, memory/disk recommendations. Backend and agent are fully wired; ⚠ **no UI surfaces it** — `GET /api/agent/recommendations` has no caller anywhere in the frontend, despite this row's own former claim that it was reachable via Settings | proxied to agent | `diagnostics.rs` | none |
...
| HTTP routes | 837 (544 backend + 293 agent) | derived | every commit |
| Regression-pin assertions | 4333 (130 suites) | derived | every commit |
| Frontend pages | 53 | derived | every commit |
| DB migrations | 132 | derived | every commit |
