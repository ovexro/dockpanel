    ...
    <div className="p-6 lg:p-8">
        ...
        <form onSubmit={handleCreate} className="bg-dark-800 rounded-xl border border-dark-500 p-5 mb-6">
          ...
          <div className="flex items-end gap-3">
            <div className="flex-1">
              <label className="block text-xs font-medium text-dark-200 mb-1">Team Name</label>
              <input type="text" value={teamName} onChange={(e) => setTeamName(e.target.value)} required placeholder="e.g. Engineering" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-rust-500 focus:border-rust-500 outline-none" />
            </div>
      ...
      {teams.length === 0 ? (
        ...
        <div className="space-y-4">
          {teams.map((t) => (
            <div key={t.id} className="bg-dark-800 rounded-xl border border-dark-500 p-5">
                ...
                {t.members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between py-1.5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-dark-100">{m.email}</span>
