...
export default function LayoutSwitcher({ variant = "dark" }: Props) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className={`absolute bottom-full mb-1 left-0 w-52 rounded-lg shadow-xl overflow-hidden z-50 ${
          ...
          {layouts.map(l => (
            ...
            >
              ...
              <div className={`text-xs ${isDark ? "text-dark-500" : "text-zinc-400"}`}>{l.desc}</div>
            </button>
          ))}
        </div>
      )}
